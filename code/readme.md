# AI Support Triage Agent - Source Code
...
### Intelligence & RAG
* **`retriever.py`**  
  The intelligence backbone implementing the Hybrid RAG system. It combines `scikit-learn`'s TF-IDF for exact-keyword (sparse) retrieval with Azure OpenAI's `text-embedding-3-small` for semantic (dense) retrieval. It features intelligent chunking, 8,000-character truncation to prevent token overflow, and automatic caching logic.
* **`build_index.py`**  
...
* **`llm.py`**  
  Manages communication with the Azure OpenAI Chat model (`gpt-4o`/`gpt-4o-mini`). Contains the heavily engineered system prompt that forces the agent to extract structured JSON, securely reason through sparse context, and aggressively minimize unnecessary human escalations.
